...
from config.logging_config import setup_logging, setup_verbose, setup_debug, get_logger
...
from src.analysis import KMeansClusterer, DBSCANClusterer, ClusterOptimizer, ClusterStatistics
...
def generate_properties(
    ...
    generator = SyntheticDataGenerator(
        ...
        district_polygons=gdf_districts
    )
...
def run_clustering(
    ...
    Returns:
        Tuple of (clustered_df, stats_df, metrics, elbow_data, silhouette_data)
    """
    if logger is None:
        logger = get_logger(__name__)
...
def create_visualizations(
    ...
    Args:
        ...
        silhouette_data: Data for silhouette plot
        debug: Enable debug logging
        logger: Logger instance
        
    Returns:
        Tuple of (map_path, report_path, plot_paths)
    """
    if logger is None:
        logger = get_logger(__name__)
    ...
    map_creator.create_district_price_map(df, output_path=district_price_map_path)
    logger.info(f"District price map saved to: {district_price_map_path}")
    
    # === 2. CREATE ALL STATIC PLOTS ===
    logger.info("Creating analysis plots (9 types)...")
    plot_creator = PlotCreator()
    
    # Use create_all_plots() for all 9 plot types
...
def print_summary(
    ...
    for cluster_id in cluster_prices.index:
        ...
        count = len(df[df["cluster"] == cluster_id])
        logger.info(f"  C{cluster_id}: {count} properties, avg {avg_price:,.0f} PLN/m²")
...
def main():
    ...
    try:
        ...
        fetcher, gdf_districts = fetch_district_boundaries(
        ...
        df = process_data(df, debug=args.debug, logger=logger)
