...
class SyntheticDataGenerator:
    ...
    def _generate_point_in_polygon(
        self, polygon: Polygon, max_attempts: int = 100
    ) -> Tuple[float, float]:
        """
    ...
    def _generate_coordinates_in_district(
        self, district: str, distance_modifier: float = 1.0
    ) -> Tuple[float, float]:
        """
        ...
        if district in self.district_centers:
            ...
            return self._generate_coordinates_fallback(
                center, radius_km=2.0, distance_modifier=distance_modifier
            )
    ...
    def generate_property(
        ...
        if force_district:
            ...
            if (
                district not in self.district_centers
                and district not in self._district_polygons
            ):
        ...
        lat, lon = self._generate_coordinates_in_district(
            district, distance_modifier=seg_config["distance_modifier"]
        )

        # Calculate distance from Warsaw center
        distance = self._haversine_distance(lat, lon, self.center[0], self.center[1])
    ...
    def generate_with_district_balance(
        self, n_samples: Optional[int] = None, min_per_district: int = 15
    ) -> pd.DataFrame:
        """
        ...
        all_districts = set(self._district_polygons.keys()) | set(
            self.district_centers.keys()
        )
        ...
        for district in sorted(all_districts):
            ...
            for _ in range(min_per_district):
                prop = self.generate_property(
                    property_id, segment=district_segment, force_district=district
                )
