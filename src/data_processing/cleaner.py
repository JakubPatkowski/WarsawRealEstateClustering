...
class DataCleaner:
    ...
    def clean(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self._cleaning_report = {"original_rows": original_len, "steps": []}
    ...
    def _remove_duplicates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if removed > 0:
            ...
            self._cleaning_report["steps"].append(f"Removed {removed} duplicates")
    ...
    def _remove_outliers_zscore(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate mask first for clarity
        mask = np.abs(z_scores) <= self.outlier_threshold

        # Explicitly cast the result to pd.DataFrame
...
class FeatureTransformer:
    ...
    def fit(self, df: pd.DataFrame) -> "FeatureTransformer":
        ...
        if self.scaling_method == "standard":
            # Use local variables for calculation (type: np.ndarray)
            means = data.mean(axis=0)
            stds = data.std(axis=0)

            # Now 'stds' is definitely an array, so np.where is happy
            ...
            # Math works here because 'maxs' and 'mins' are definitely arrays
            ranges = maxs - mins
            ranges = np.where(ranges == 0, 1, ranges)

            # Assign to instance variables
            self._mins = mins
            # Store the adjusted maxs (to ensure range is never 0)
            self._maxs = mins + ranges
    ...
    def transform(self, df: pd.DataFrame) -> np.ndarray:
        ...
        # Explicitly hint that data is an array (optional, but helpful)
        ...
        if self.scaling_method == "standard":
            ...
            # Fix Line 362: Cast return value
            return cast(np.ndarray, (data - self._means) / self._stds)

        elif self.scaling_method == "minmax":
            if self._mins is None or self._maxs is None:
                raise ValueError("Fitting failed: mins/maxs are None")

            return cast(np.ndarray, (data - self._mins) / (self._maxs - self._mins))
