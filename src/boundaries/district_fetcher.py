...
def clean_multipolygon(
    ...
    significant_parts = [
        part for part, area in zip(parts, areas)
...
class DistrictFetcher:
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized DistrictFetcher with timeout={api_timeout}s"
        )
    ...
    def fetch_from_api(self) -> Optional[Dict[str, Any]]:
        ...
        Uses simplified query WITHOUT recursion to avoid duplicates.
    ...
    def process_raw_data(
        ...
        if data is None:
            if self.raw_data is None:
                raise ValueError(
                    "No data to process. Call fetch_from_api() first."
                )
            data = self.raw_data
        ...
        for element in elements:
            ...
            if verbose:
                logger.debug(
                    f"  ✓ {name}: {geom.geom_type} ({len(lines)} segments)"
                )
    ...
    def get_district_by_point(
        ...
        Returns:
            District name or None if point is outside all districts
    ...
    def get_summary(self) -> str:
        ...
        for _, row in self.gdf.sort_values("area_km2", ascending=False).iterrows():
            error_str = f"{row['area_error_pct']:.1f}%" if pd.notna(row['area_error_pct']) else "N/A"
            lines.append(
