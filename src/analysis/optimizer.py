...
class ClusterOptimizer:
    ...
    def fit(self, X: np.ndarray) -> "ClusterOptimizer":
        ...
        for k in k_values:
            ...
            logger.debug(f"  k={k}: inertia={inertia:.2f}, silhouette={silhouette:.4f}")
    ...
    def _find_elbow_k(self) -> None:
        ...
        # Fix 1: Explicitly convert to numpy array with float type
        inertias = self._results["inertia"].to_numpy(dtype=float)
        k_values = self._results["k"].to_numpy()
        ...
        if len(diffs2) > 0:
            # Fix 2: Cast numpy int64 to Python int before math operations
            elbow_idx = int(np.argmax(diffs2)) + 1  # +1 to account for diff offset

            # Ensure within valid range
            # Now all inputs to max/min are standard Python ints
    ...
    def get_elbow_data(self) -> Tuple[List[int], List[float]]:
        ...
        return (self._results["k"].tolist(), self._results["inertia"].tolist())
    ...
    def get_silhouette_data(self) -> Tuple[List[int], List[float]]:
        ...
        return (self._results["k"].tolist(), self._results["silhouette"].tolist())
